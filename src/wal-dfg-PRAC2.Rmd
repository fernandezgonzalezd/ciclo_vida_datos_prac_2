...
```{r, analisis_datos,eval=TRUE,echo=TRUE}


par(mfrow=c(2,2))
for(i in 1:ncol(eliminated_outliers)) {
  if (is.numeric(eliminated_outliers[,i])){
    qqnorm(eliminated_outliers[,i],main = paste("Normal Q-Q Plot ",colnames(eliminated_outliers)[i]))
    qqline(eliminated_outliers[,i],col="red")
    hist(eliminated_outliers[,i], 
      main=paste("Hist. for ", colnames(eliminated_outliers)[i]), 
